import { Link, BrowserRouter as Router, Switch, Route, useParams, Redirect } from "react-router-dom";
    ...
    <Router>
      <section className="menu-category-display">
        <div className="container">
          <a href="/menu" className="menu-return btn">
            Go Back
          </a>
          <Switch>
            <Route path="/menu/category/:category/:item" children={<MenuItemDisplay menuCategories={menuCategories} />} />
          </Switch>
            ...
            {categories.map((category) => {
              if (category.name === id) {
                return category.menu.map((item, idx) => (
                  <Link to={`/menu/category/${category.name}/${item.id}`} className="menu-card-link" key={idx}>
