...
function MenuOverview({ menuCategories }) {
  ...
  const menuStats = [
    {
      stat: totalMenuItems(),
      subtitle: "Menu Choices",
    },
    {
      stat: "FREE",
      subtitle: "Birthday Meals",
    },
    {
      stat: 839,
      subtitle: "Reviews",
        ...
        <article id="menu-statistics" className="triple-col">
          <div className="grid-container">
            {menuStats.map((item, idx) => (
              <section className="menu-statistic" key={idx}>
                <h2 className="stat-num">{item.stat}</h2>
                <h4 className="stat-subtitle">{item.subtitle}</h4>
              </section>
            ))}
          </div>
        </article>
